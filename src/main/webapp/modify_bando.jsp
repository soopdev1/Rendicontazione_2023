...
<%
    if (session.getAttribute("username") == null) {
        ...
        if (!Action.isVisibile(stat, "modify_bando.jsp")) {
            ...
            String diff = String.valueOf(Double.parseDouble(b.getBudget()) - Double.parseDouble(b.getBudget_previsione()));
%>
<!DOCTYPE html>
<html>
    <head>
    ...
    <script type="text/javascript">
        ...
        function ctrlForm() {
            ...
            var tipo = $("#tipo").val();
            var budget = $("#budgets").val().replace(",", ".");
            var datafine = $("#datafine").val();
            var budgetp = $("#budgetp").val().replace(",", ".");
            var differenza = Number($("#differenza").val().replace(",", "."));

            var flag_sportello = document.getElementById("flag_sportello").checked;
            var somma = $("#budget").val() + "." + $("#centesimi").val();
            if (document.getElementById("piu_meno").checked == false) {
                somma = Number(-somma);
            }

            if (somma != "." && somma != "0.00" && somma != "0." && somma != ".00") {
                var newbudget = Number(budgetp) + Number(somma);
            } else {
                var newbudget = Number(budgetp);
            }
            ...
            if (totale != "0" && (Number(newbudget) < 0)) {
                document.getElementById("div_somma").style.display = "block";
                $('#span').html("<strong>&bull; L'importo del nuovo budget (" + currencyFormat(newbudget) + ") è minore del budget già stanziato.</strong><br>");
                $("#div_budget").attr("class", "col-md-2 has-error");
                $("#div_centesimi").attr("class", "col-md-1 has-error");
                $("#div_budgetprev").attr("class", "col-md-2 has-warning");
                err = true;
            } else {
                $("#div_budget").attr("class", "col-md-2 has-success");
                $("#div_centesimi").attr("class", "col-md-1 has-success");
            }

            if (Number(newbudget) < 0 && budgetp != budget) {
                $("#div_budget").attr("class", "col-md-2 has-error");
                $("#div_centesimi").attr("class", "col-md-1 has-error");
                $("#div_budgetprev").attr("class", "col-md-2 has-warning");
                document.getElementById("div_somma").style.display = "block";
                $('#span2').html("<strong>&bull; L'importo del nuovo budget deve essere maggiore o uguale di " + currencyFormat(differenza) + " (differenza tra budget stanziato e budget previsionale).</strong>");
                err = true;
            }
                                ...
                                <form class="form-horizontal" action="<%="OperazioniRegional?type=2"%>" method="post" enctype="multipart/form-data" onsubmit="return ctrlForm();" >
                                    <div class="form-body">
                                        <div class="form-group">
                                            ...
                                            <input type="hidden" name="differenza" id="differenza" value="<%=diff%>"/>
                                            <%
                                        ...
                                        <div class="form-group">
                                            <div class="col-md-2" id="div_budget" style=" padding-right: 0px; width: 16%;">
                                                <label>Incremento/decremento</label>
                                                <div class="input-group"> 
                                                    <span class="input-group-addon" style="background-color: #eef1f5;">€</span>
                                                    <input class="form-control" style="direction: rtl;" type="number" id="budget" name="budget" step="1" placeholder="0000" min="0"></div>
                                            </div>
                                            ...
                                            <div class="col-md-1" id="div_centesimi" style=" padding-left: 0px; width: 8%; ">
                                                <label>&nbsp;</label>
                                                <input class="form-control" type="number" id="centesimi" name="centesimi" placeholder="00" min="0" max="99" step="1" >
                                            </div>
                                            <div class="col-md-1" style="padding-left: 0px;"> 
                                                <label>&nbsp;</label><br/>
                                                <input type="checkbox" class="make-switch form-control" checked name="piu_meno" id="piu_meno"  data-on-text="<i class='fa fa-plus-circle' style='font-size: 18px; '></i>" data-on-color="success" data-off-color="danger" data-off-text="<i class='fa fa-minus-circle' style='font-size: 18px;'></i>" >
                                            </div>
                                            <div class="col-md-2" id="div_budgets" style=" padding-right: 0px; width: 18%;">
                                                <label>Budget stanziato</label>
                                                <div class="input-group"> 
                                                    <span class="input-group-addon" style="background-color: #eef1f5;">€</span>
                                                    <input class="form-control" readonly style="direction: rtl;" name="budgets" id="budgets" type="text" value="<%=budget + "," + cents%>" ></div>
                                            </div>
                                            <div class="col-md-2" id="div_budgetprev" style=" padding-right: 0px; width: 18%;">
                                            ...
                                            <div class="col-md-2" id="div_budgetatt" style=" padding-right: 0px; width: 18%;">
