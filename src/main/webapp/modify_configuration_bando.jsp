...
<html>
    ...
    <script type="text/javascript">
        ...
        <%for (int j = 0; j < valori.size(); j++) {%>
            ...
            if (document.getElementById("piu_meno<%=valori.get(j)[0]%>").checked == false) {
                provvisorio = Number(-provvisorio);
            }
            var budgetstanziato = $("#budgets<%=valori.get(j)[0]%>").val().replace(",", ".");
            var budgetprovvisorio = $("#budgetp<%=valori.get(j)[0]%>").val().replace(",", ".");
            //var budgetattuale = $("budgeta<%=valori.get(j)[0]%>").val().replace(",",".");
            // var differenza = $("#differenza<%=valori.get(j)[0]%>").val().replace(",", ".");
            if (Number(budgetprovvisorio) + Number(provvisorio)<0) {
                $("#div_budgets<%=valori.get(j)[0]%>").attr("class", "col-md-2 has-error");
            ...
            if (provvisorio != "." && provvisorio != "0.00" && provvisorio != "0." && provvisorio != ".00") {
                somma += (Number(budgetstanziato) + Number(provvisorio));
            } else {
                somma += Number(budgetstanziato);
            }
        <%}%>
                ...
                <div class="page-content">                    
                    ...
                    <div class="row">
                        <div class="portlet light bordered ">
                            ...
                            <%}
                                if (esitomod.equals("") && !b.getStato().equals("I")) {%>
                                ...
                                <form class="form-horizontal" action="<%="OperazioniRegional?type=4"%>" method="post" enctype="multipart/form-data" onsubmit="return ctrlForm();" >
                                    <div class="form-body">
                                        <%for (int i = 0; i < valori.size(); i++) {
                                                ...
                                                String budgeta = valori.get(i)[4].substring(0, valori.get(i)[4].lastIndexOf("."));
                                                String centsa = valori.get(i)[4].substring(valori.get(i)[4].lastIndexOf(".") + 1);
                                                if (centsa.length() == 1) {
                                                    centsa += "0";
                                                }
                                        ...
                                        <div class="portlet light bordered" style="color:#3598DC;">
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-2 "  id="div_budgeta<%=valori.get(i)[0]%>">
                                                    <label>Budget attuale</label>
                                                    <div class="input-group"> 
                                                        <span class="input-group-addon" style="background-color: #eef1f5;">€</span>
                                                        <input class="form-control" readonly style="direction: rtl;" name="budgeta<%=valori.get(i)[0]%>" id="budgeta<%=valori.get(i)[0]%>" type="text" value="<%=budgeta + "," + centsa%>" >
                                                    </div>
                                                </div>
                                                <div class="col-md-2 " id="div_budgets<%=valori.get(i)[0]%>">
                                                    <label>Budget stanziato</label>
                                                    <div class="input-group"> 
                                                        <span class="input-group-addon" style="background-color: #eef1f5;">€</span>
                                                        <input class="form-control" readonly style="direction: rtl;" name="budgets<%=valori.get(i)[0]%>" id="budgets<%=valori.get(i)[0]%>" type="text" value="<%=budget + "," + cents%>" >
                                                    </div>
                                                </div>
                                                <div class="col-md-4">
                                                    <div class="col-md-1">&nbsp;
                                                    </div>
                                                    <div class="col-md-5" id="div_incremento<%=valori.get(i)[0]%>" style=" padding-right: 0px;">
                                                        <label>Incremento/decremento</label>
                                                        <div class="input-group"> 
                                                            <span class="input-group-addon" style="background-color: #eef1f5;">€</span>
                                                            <input class="form-control" style="direction: rtl;" type="number" id="budget<%=valori.get(i)[0]%>" name="budget<%=valori.get(i)[0]%>"  placeholder="0000"></div>
                                                    </div>
                                                    <div class="col-md-1" style=" padding-left: 5px;padding-right: 10px; width: 1%; font-size: 20px;">  <label>&nbsp;</label><br>
                                                        ,
                                                    </div>
                                                    <div class="col-md-3" id="div_centesimi<%=valori.get(i)[0]%>" style=" padding-left: 0px; width: 22%; ">
                                                        <label>&nbsp;</label>
                                                        <input class="form-control quantity" type="number" id="centesimi<%=valori.get(i)[0]%>" name="centesimi<%=valori.get(i)[0]%>"  min="0" max="99" placeholder="00" step="1" >
                                                    </div>
                                                    <div class="col-md-2" style="padding-left: 0px;"> 
                                                        <label>&nbsp;</label><br/>
                                                        <input type="checkbox" class="make-switch form-control" checked name="piu_meno<%=valori.get(i)[0]%>" id="piu_meno<%=valori.get(i)[0]%>"  data-on-text="<i class='fa fa-plus-circle' style='font-size: 18px; '></i>" data-on-color="success" data-off-color="danger" data-off-text="<i class='fa fa-minus-circle' style='font-size: 18px;'></i>" >
                                                    </div>
                                                </div>
                            ...
                            <%} else if (b.getStato().equals("I")) {%>
                            <div class="alert bg-yellow-gold alert-dismissible " style="text-align: center;color: white">
                                <button type="button" class="close" data-close="alert"></button>
                                <span><strong>Bando Inattivo!</strong></span>
                            </div>
                            <%}%>
                        </div>
                    </div>
