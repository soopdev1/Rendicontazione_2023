...
<%
    if (session.getAttribute("username") == null) {
        ...
        if (!Action.isVisibile(stat, "modifyConvenzione.jsp")) {
...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            function ctrlForm() {
                ...
                if (codice == "") {
                    $("#div_codice").attr("class", "col-md-3 has-error");
                    err = true;
                } else {
                    $("#div_codice").attr("class", "col-md-3 has-success");
                }
                if (politica == null || politica == "...") {
                    $("#div_politica").attr("class", "col-md-3 has-error");
                    err = true;
                } else {
                    $("#div_politica").attr("class", "col-md-3 has-success");
                }
                if (bando == null || bando == "...") {
                    $("#div_bando").attr("class", "col-md-3 has-error");
                    err = true;
                } else {
                    $("#div_bando").attr("class", "col-md-3 has-success");
                }
                if (from == "" || to == "") {
    ...
    <body class="page-full-width page-content-white">
        ...
        <div class="page-container">
                ...
                <div class="page-content">                    
                    ...
                    <div class="row">
                        ...
                        <div class="alert bg-green-jungle alert-dismissible " style="text-align: center;color: white">
                            <button type="button" class="close" data-close="alert"></button>
                            <span><strong>Convenzione aggiornata con successo!</strong></span>
                        </div>
                        <%} else if (esitoedit.equals("")) {
                            Convenzione t = Action.findConvenzioneById((String) request.getParameter("idconvenzione"));
                            ArrayList<String[]> politica = Action.getListTipoPolitica("2");
                            selected_p = t.getPolitica();
                        %>
                        <div class="portlet light bordered ">
                            <div class="portlet-title">
                                <div class="caption">
                                    <span class="caption-subject font-blue bold">
                                        <h3 class="page-title"><strong> CONVENZIONE: </strong><small> <%=t.getCodice()%></small> </h3>                                    
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="portlet-body form">
                                <form class="form-horizontal" action="<%="OperazioniEnte?type=5"%>" method="post" name="form" accept-charset="ISO-8859-1" onsubmit="return ctrlForm();" enctype="multipart/form-data">
                                    <input type="hidden" value="<%=t.getId()%>" name="idconvenzione">
                                    <input type="hidden" value="<%=t.getFile()%>" name="file_old">
                                    <div class="form-body">
                                        <div class="form-group">
                                            <div class="col-md-3" id="div_codice">
                                                <label>Num.Repertorio</label><label type="text" style="color: red" >*</label>
                                                <input type="text" class="form-control" id="codice" name="codice" value="<%=t.getCodice()%>">
                                            </div>
                                            <div class="col-md-3" id='div_date'>
                                                <label>Range Date</label><label type="text" style="color: red" >*</label>
                                                <div class="input-group date-picker input-daterange" data-date="01/01/2000" data-date-format="dd/mm/yyyy">
                                                    <input type="text" class="form-control" name="from" id="from" value="<%=t.getInizio()%>">
                                                    <span class="input-group-addon"> a </span>
                                                    <input type="text" class="form-control" name="to" id="to" value="<%=t.getFine()%>"> 
                                                </div>
                                            </div>
                                            <div class="col-md-3" id="div_bando">
                                                <label>Bando</label><label type="text" style="color: red" >*</label>
                                                <select class="form-control select2-allow-clear" id="bando" name="bando">
                                                    <option value="..." selected>...</option>
                                                    <% for (int j = 0; j < tipoBando.size(); j++) {%>
                                                    <%if (tipoBando.get(j)[0].equals(String.valueOf(t.getBando()))) {%>
                                                    <option selected value="<%=tipoBando.get(j)[0]%>"><%=tipoBando.get(j)[1]%></option>
                                                    <%} else {%>
                                            ...
                                            <div class="col-md-3" id="div_politica" style='display: none'>
                                                <label>Politica</label><label type="text" style="color: red" >*</label>
                                                <select class="form-control select2-allow-clear" id="politica" name="politica" style='width: 100%'>
                                                    <option value="..." selected>...</option>
                                                </select>
                                            </div>
                                        </div>
            ...
            <script type="text/javascript">
                $('#bando').change(function () {
                    populatepolitica();
                });

                function populatepolitica() {
                    var selected = '<%=selected_p%>';
                    $('#politica').empty();
                    if ($('#bando').val() != '...' && $('#bando').val() != null) {
                        $('#div_politica').css('display', 'block');
                        $.get("OperazioniEnte?type=31&bando=" + $('#bando').val(), function (response) {
                            var obj = JSON.parse(response);
                            $('#politica').append("<option value='...'>...</option>");
                            for (var i = 0; i < obj.length; i++) {
                                if (obj[i][0] == selected) {
                                    $('#politica').append("<option selected value='" + obj[i][0] + "'>" + obj[i][1] + "</option>");
                                } else {
                                    $('#politica').append("<option value='" + obj[i][0] + "'>" + obj[i][1] + "</option>");
                                }
                            }
                        });
                    } else {
                        $('#div_politica').css('display', 'none');
                    }
                }
