...
<%
    if (session.getAttribute("username") == null) {
        ...
        if (!Action.isVisibile(stat, "regI_elaboraDT.jsp")) {
...
<html>
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function setAccettaID(id) {
                document.getElementById('confirmactive').href = 'OperazioniRegional?type=20&idregistri=' + id + "&tipo=" + document.getElementById(id + '_pol').value
                        + "&ente=<%=ente%>&from=<%=from%>&to=<%=to%>&nome=<%=nome%>&cognome=<%=cognome%>&cf=<%=cf%>";
            }

            function setRifiutaID(id) {
                document.getElementById('confirmreject').href = 'OperazioniRegional?type=21&idregistri=' + id + "&tipo=" + document.getElementById(id + '_pol').value
                        + "&ente=<%=ente%>&from=<%=from%>&to=<%=to%>&nome=<%=nome%>&cognome=<%=cognome%>&cf=<%=cf%>";
            }
            ...
            function ctrlForm() {
                ...
                var cf = $("#cf").val();
                var err = false;
                if ((ente == null || ente == "...") && from == "" && to == "" && cf == "" && nome == "" && cognome == "" && politica == "") {
            ...
            function submitScarto() {
                ...
                if (motivo1 == null || motivo1 == "..." || motivo1 == "") {
                    ...
                    if (motivo2 == "") {
                        ...
                        document.getElementById('conferma_scarta').href = "OperazioniRegional?type=23&motivo=" + encodeURIComponent(motivo2) + "&idregistri=" + $('#id_s').val() + '&tipo=C06'
                                + "&ente=<%=ente%>&from=<%=from%>&to=<%=to%>&nome=<%=nome%>&cognome=<%=cognome%>&cf=<%=cf%>";
                    }
                } else {
                    document.getElementById('conferma_scarta').href = "OperazioniRegional?type=23&motivo=" + encodeURIComponent(motivo1) + "&idregistri=" + $('#id_s').val() + '&tipo=C06'
                            + "&ente=<%=ente%>&from=<%=from%>&to=<%=to%>&nome=<%=nome%>&cognome=<%=cognome%>&cf=<%=cf%>";
                }
    ...
    <body class="page-header-fixed page-sidebar-closed-hide-logo page-content-white">
        <form action="OperazioniRegional?type=29" method="post" id="form2" accept-charset="ISO-8859-1" style="display: none">
            <input type="hidden" name="data" id="data">
            <input type="hidden" name="tipo" value="C06">
        ...
        <div class="modal fade bs-modal-lg" id="large" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header bg-red-thunderbird  alert-dismissible">
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header bg-green" style="color: white;">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true" onclick="return dismiss('accettamodal');"></button>
                        <h4 class="modal-title">Conferma Convalida Registri</h4>
                    </div>
                    <div class="modal-body" > 
                        <div id="largetext2"></div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn red-thunderbird btn-outline" data-dismiss="modal" onclick="return dismiss('accettamodal');">Chiudi</button>
                        <a class="btn btn-outline green-jungle large" id="confirmdisactive" onclick="return creaDomanda()"><i class="fa fa-upload"></i> Convalida</a> 
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header alert-warning">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                        <h4 class="modal-title">Rifiuta Registro</h4>
                    </div>
                    <div class="modal-body" id="largetext">                       
                        <label class="text"> Sicuro di voler rifiutare questo registro? </label>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn large btn-danger" data-dismiss="modal" onclick="return dismiss('rifiutamodal');">Chiudi</button>
                        <a class="btn btn-warning large " id="confirmreject" onclick="">Conferma</a> 
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content ">
                    <div class="modal-header   alert-danger ">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                        <h4 class="modal-title">Scarta Definitivamente Registro</h4>
                    </div>
        ...
        <div class="page-container">
            <!-- BEGIN MENU -->
            <%@ include file="menu_regional/menu_inpsDT.jsp"%>
            <!-- END MENU -->
            ...
            <div class="page-content-wrapper">
                ...
                <div class="page-content">
                    ...
                    <!-- END PAGE BAR -->
                    <!-- BEGIN PAGE TITLE-->                    
                    <h3 class="page-title" style="color:#2D5F8B"><strong> ACCOMPAGNAMENTO AL TIROCINIO </strong>  <small style="color:#2D5F8B"> CONSULTA </small> </h3>    
                    ...
                    <div class="portlet box red-flamingo">
                        ...
                        <div class="portlet-body form">
                            <form class="form-horizontal" action="regI_elaboraDT.jsp" method="post" name="form" accept-charset="ISO-8859-1" onsubmit="return ctrlForm();">
                                ...
                                <div class="form-body" id="div_form">
                                    ...
                                    <div class="form-group">
                                        <div class="col-md-4">
                                            ...
                                            <a href="regI_elaboraDT.jsp" class="btn red-thunderbird btn-outline"><i class="fa fa-close"></i> Reset</a>
                ...
                jQuery(document).ready(function () {
                    var dt2 = function () {
                        ...
                        g.dataTable({
                            ...
                            processing: true,
                            ajax: {
                                url: "QueryRegional?type=41&politica=C06&ente=<%=ente%>&from=<%=from%>&to=<%=to%>&nome=<%=nome%>&cognome=<%=cognome%>&cf=<%=cf%>",
                                dataSrc: "aaData",
                                type: "GET"
                            },
                            ...
                            buttons: [
                                ...
                                {className: "btn font-white btn-outline bg-green-jungle", text: "<i class='fa fa-check'></i> Convalida",
                                    action: function deleteAll(e, dt, node, config) {
                                        ...
                                        if (chkbox_checked.length === chkbox_checked2.length) {
                                            showmod1("large", "largetext", "Seleziona almeno un registro");
                                            return false;
                                        }
                                        if (chkbox_checked.length > 1) {
                                            showmod1("accettamodal", "largetext2", "Confermi l'avvenuto pagamento per " + chkbox_checked.length + " registri?");
                                        } else {
                                            showmod1("accettamodal", "largetext2", "Confermi l'avvenuto pagamento per un registro?");
                                        }
